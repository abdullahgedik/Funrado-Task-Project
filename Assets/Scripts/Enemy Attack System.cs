...
public class EnemyAttackSystem : AttackSystem
{
    ...
    public void AttackToPlayer(Collider col)
    {
        if(col.GetComponent<LevelSystem>() != null)
        {
            ...
            if (col.GetComponent<LevelSystem>().GetLevel() > this.GetLevelSystem().GetLevel())
            {
                col.GetComponent<PlayerAttackSystem>().AttackEnemy(this.gameObject.GetComponent<Collider>());
            }
        }
    }
}
